...
class Anti_cars(pygame.sprite.Sprite): # Simple base class for visible game objects.
    def __init__(self, size_of_main_screen):
        super().__init__
        self.size_of_main_screen = size_of_main_screen
        print(size_of_main_screen) # (400, 600) --> AnimatedScreen
        self.image = pygame.image.load("Enemy.png")
        self.rect = self.image.get_rect()
        self.start_pos = (random.randint(5, self.size_of_main_screen[0]), 5)
        print("st.po", self.start_pos)
        self.rect.center = self.start_pos # remember start pos of motion, sets the initial position of the enemy car.
        self.some_value = 2 # speed = 2
    ...
    def build(self, main_image): # blits the enemy car image onto the main game screen.
...
class Cars(pygame.sprite.Sprite):
    def __init__(self, size_of_main_screen):
        ...
        self.start_pos = (random.randint(5, self.size_of_main_screen[0]), self.size_of_main_screen[1] * 0.8) # OY ---> 80% пространства
        self.rect = self.image.get_rect()
        self.rect.center = self.start_pos
        self.speed = 4
        self.some_coin = pygame.image.load("coin.png") # my car wanna touch coin
        self.image_coin = pygame.transform.scale(self.some_coin, (40, 40)) # scale = resize 460x460 to 40x40
        self.rect_coin = self.image_coin.get_rect()

    def walk(self):
        if self.rect.center[0] > self.size_of_main_screen[0]:
            self.rect.center = (5, self.rect.center[1]) # перенеси обратно в начальную позицию
    ...
    def build(self, main_image):
        self.main_image = main_image
        self.main_image.blit(self.image, self.rect.center) # blit --> (какую картинку и где указывать)
...
a_car = Anti_cars(some_picture.get_size()) # AnimatedScreen
player = Cars(some_picture.get_size()) # player = child , Cars - parent class
...
FPS = 60 # изменение картинки 60 за секунду
# regulates the speed at which the game loop runs and ensures that the game runs at a consistent frame rate.
bonus = pygame.USEREVENT + 1 # bonus = появление 1tenge
pygame.time.set_timer(bonus, 1000) # каждую секунду поятвляется монета
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if event.type == difficult:
            if a_car.some_value < 8: # enemy speed < 8:
                a_car.some_value += 1
    main_screen.blit(some_picture, (0, 0))

    if bolean:
        player.coin() # class Cars, вызывать coin function, чтобы появиласб монета
        player.build_coin(some_picture)
        bolean = False

    shadow_coin = font.render(str(coins), True, pygame.Color("yellow"))
    # creates a text image shadow_coin that shows the current number of coins collected by the player, using the font object created earlier.
